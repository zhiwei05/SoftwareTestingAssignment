...
public class calculatePrintingCharge {
    ...
    public double calculateBaseCharge(String paperSize, String printType, String side, int pages, int copies) {
        ...
        if (paperSize.equalsIgnoreCase("A4")) {
            if (printType.equalsIgnoreCase("Black & White")) rate = side.contains("Single") ? 0.20 : 0.18;
            else rate = side.contains("Single") ? 0.80 : 0.75;
        } else if (paperSize.equalsIgnoreCase("A3")) {
            if (printType.equalsIgnoreCase("Black & White")) rate = side.contains("Single") ? 0.40 : 0.35;
            else rate = side.contains("Single") ? 1.50 : 1.40;
        } else if (paperSize.equalsIgnoreCase("A5")) {
            if (printType.equalsIgnoreCase("Black & White")) rate = side.contains("Single") ? 0.15 : 0.13;
            else rate = side.contains("Single") ? 0.60 : 0.55;
        } else {
    ...
    public double calculateTotalCharge(printOrder order) {
        ...
        double base = calculateBaseCharge(order.getPaperSize(), order.getPrintType(), 
                                          order.getPrintingSide(), order.getNumberOfPages(), order.getNumberOfCopies());
        double opt = calculateOptionalServiceCharge(order.getBindingOption(), 
                                                     order.isLamination(), order.isExpressPrinting(), 
                                                     order.getNumberOfPages(), order.getNumberOfCopies());
        ...
        order.setDiscountAmount(subtotal - discountedTotal);
        order.setTotalCharge(discountedTotal);
